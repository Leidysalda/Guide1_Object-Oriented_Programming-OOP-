...
public class Parqueadero {
    ...
    public void avanzarHora() {
        horaActual++;
        if (horaActual == (HORA_CIERRE + 1) && (horaActual > 21 || horaActual < 5)) {
            abierto = false;
            horaActual = HORA_INICIAL;
        }
        else {
    ...
    public String metodo1() {
        ...
        for (Puesto p : puestos) {
            if (p.estaOcupado()) {
                ...
                if (compararTiempo < carro.darTiempoEnParqueadero(horaActual)) {
                    compararTiempo = carro.darTiempoEnParqueadero(horaActual);
                    placa = carro.darPlaca();
                }
            }
        }
        return "El vehículo con placa " + placa + " tiene más tiempo en el parqueadero, lleva " + compararTiempo + "horas";
    }
